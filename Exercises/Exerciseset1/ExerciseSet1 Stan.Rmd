...
1. in dataSet1.R in line 8 we have a list mytickers where we add the Apple listing symbol AAPL to be retrieved:
```
mytickers = c("SPY", "AAPL")
```
then in the for loop starting at line 20 we have:
```
  for (ticker_index in 1:length(mytickers)){
    ...
    ... 
```
APPL data is sourced in the second iteration of the for loop. 
Then on line 35 the list StockReturns is populated with Apple stock returns and on line 45 the list is stored to DataSet1.Rdata file
<br>
<br>
2. 
The cumulative returns of the Apple during this period is `r round(100*sum(StockReturns[,2]),1)`%.<br>
The average daily returns of the Apple index during this period is `r round(100*mean(StockReturns[,2]),3)`%;<br>
The standard deviation of the daily returns of the Apple index during this period is `r round(100*sd(StockReturns[,2]),3)`%;<br>
<br>
3.
Yes, we have to edit the source code of dataSet1.R in the following manner. We add "YHOO" to mystickers list:
```
mytickers = c("SPY", "AAPL","YHOO")
```
Then we re-source dataSet1.R what will result in results of YHOO integrated in the DataSet1.Rdata file. Yahoo will be yet another column in the table StockReturns.<br>
The cumulative returns of the Yahoo! during this period is `r round(100*sum(StockReturns[,8]),1)`%.<br>
The average daily returns of the Yahoo! index during this period is `r round(100*mean(StockReturns[,8]),3)`%;<br>
The standard deviation of the daily returns of the Yahoo! index during this period is `r round(100*sd(StockReturns[,8]),3)`%;<br>
<br>
...
<br>
1.
Use: dim(transposedData) or
nrow(transposedData) and ncol(transposedData) 
<br>
2.<br>
```
transposedData = t(StockReturns)<br>
portfolio <- t(apply(transposedData, 2, mean))<br>
names(portfolio) <- colnames(transposedData)<br>
pnl_plot(portfolio)<br>
```
<br>
...
**Your Answers here:**
<br>
1. In dataSet1.R we have to change the startDate from "=2001-01-01" to "=2000-12-28" as the StockReturns is calculated using StockPrices from previous days.
<br>
2. Creating an equally wieghted portfolio of: AAPL, GOOG, TSLA, FB and ploting it over period from 2012-12-06 to 2017-01-13
<br>
```
portfolio2 = apply(StockReturns[3000:nrow(StockReturns),c(2:3,5:6)],1,mean)
names(portfolio2) <- rownames(StockReturns[3000:nrow(StockReturns),c(2:3,5:6)])
pnl_plot(portfolio2)
```
<br>
```{r echo=FALSE, comment=NA, warning=FALSE, message=FALSE,results='asis',fig.align='center', fig.height=4,fig.width= 6, fig=TRUE}
...
1. Once you write and read the data as described above, what happens when you run this command in the console of the RStudio: `sum(myData != StockReturns[1:20,c("SPY","AAPL")])`
...
```
<br>
data entries have been formatted when saved to and retrieved from .csv file. The retrieved data (myData) is table of data.frame format with 1 column and 20 lignes, because the values of data entries for each variable are collated and saved as text. StockReturns[1:20,c("SPY","AAPL")] is a matrix and has 2 columns of numerical data plus a date column. 
<br>
2. We are adding a data.frame of 1 col and 20 lines where data entries are text with a matrix of 2 columns (plus date) and 40 lines where data entries are numbers. There should be an error message.
<br>
